<template>
    <div class="evaluate">
		...
		<div class="cartgoodsdetail" v-for="(item,index) in evaluateGoodsList">
...
export default {
    ...
    methods : {
        back(){
			...
			...Vuex.mapActions({
				handleEvaluateGoods:"mine/handleEvaluateGoods"
				
			})
    },
    computed:{
			...Vuex.mapState({
				evaluateGoodsList:state=>state.mine.evaluateGoodsList
			})
	},
	filters:{
			sum:function(val){
...
.evaluate>.content{height:100%;background: red}
.wrapper{
		overflow: hidden;
		position: absolute;
		top: 2.2rem;
